...
const MergeVariantsModal: React.FC<MergeVariantsModalProps> = ({
  isOpen,
  onClose,
  variants,
  onMerge,
  ...
  const [precheckDuplicates, setPrecheckDuplicates] = useState<Array<{
    id: number;
    name: string;
  }> | null>(null);
  ...
  const handleSourceToggle = (variantCode: string) => {
    setSourceVariants((prev) =>
      prev.includes(variantCode)
        ? prev.filter((code) => code !== variantCode)
        : [...prev, variantCode]
    );
  };
  ...
  return (
    <div className='bg-opacity-50 fixed inset-0 z-50 flex items-center justify-center bg-black'>
      <div className='max-h-[80vh] w-full max-w-2xl overflow-y-auto rounded-lg bg-white p-6'>
        <h2 className='mb-4 text-xl font-bold'>상품 코드 병합</h2>
        ...
        <div className='mb-6'>
          ...
          <div className='max-h-48 overflow-y-auto rounded-md border border-gray-300 p-3'>
            {availableForSource.length === 0 ? (
              <p className='text-sm text-gray-500'>선택 가능한 상품이 없습니다.</p>
            ) : (
              availableForSource.map((variant) => (
                <label key={variant.variant_code} className='mb-2 flex cursor-pointer items-center'>
                  <input
                    type='checkbox'
        ...
        <div className='mb-6 rounded-md border border-yellow-200 bg-yellow-50 p-4'>
          <div className='flex items-start'>
            <FaExclamationTriangle className='mt-1 mr-2 flex-shrink-0 text-yellow-600' />
            <div className='text-sm text-yellow-800'>
              <p className='mb-1 font-semibold'>⚠️ 주의사항:</p>
