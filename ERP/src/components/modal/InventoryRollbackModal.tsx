...
import {
  ...
  useRollbackInventory,
} from '../../hooks/queries/useInventorySnapshots';
          ...
          <div className='p-6'>
            <div className='mb-4 rounded-lg border border-amber-200 bg-amber-50 p-4'>
              <div className='flex items-start'>
                <FiAlertTriangle className='mt-1 mr-3 flex-shrink-0 text-amber-600' />
                <div className='text-sm text-amber-800'>
                  <p className='mb-2 font-medium'>⚠️ 주의사항</p>
            ...
            {selectedSnapshot && (
              <div className='mb-4 rounded-lg bg-gray-50 p-4'>
                <h4 className='mb-2 font-medium text-gray-900'>롤백할 스냅샷</h4>
                <div className='space-y-1 text-sm text-gray-600'>
                  ...
                  <p>
                    <span className='font-medium'>사유:</span> {selectedSnapshot.reason}
                  </p>
                  {selectedSnapshot.meta?.filename && (
                    <p>
                      <span className='font-medium'>파일:</span> {selectedSnapshot.meta.filename}
                    </p>
                  )}
                </div>
              </div>
            ...
            <div className='mb-6'>
              <label className='mb-2 block text-sm font-medium text-gray-700'>롤백 사유</label>
              <textarea
      ...
      <div className='mx-4 flex max-h-[90vh] w-full max-w-5xl flex-col rounded-lg bg-white shadow-lg'>
        ...
        <div className='border-b border-gray-200 p-6'>
          <div className='rounded-lg border border-blue-200 bg-blue-50 p-4'>
            <div className='flex items-start'>
              <FaHistory className='mt-1 mr-3 flex-shrink-0 text-blue-600' />
              <div className='text-sm text-blue-800'>
                <p className='mb-1 font-medium'>📄 재고 백업 기록</p>
                <p className='text-xs'>
                  데이터 업로드 전에 자동으로 저장된 재고 현황입니다. 문제가 발생했을 때 이전 상태로
                  되돌릴 수 있습니다.
                </p>
              </div>
            </div>
                ...
                <tbody>
                  {snapshots.map((snapshot, index) => (
                    <tr
                      ...
                      <td className='min-w-[60px] px-4 py-3 text-left text-sm font-medium'>
                        #{snapshot.id}
                      </td>
                      <td className='min-w-[140px] px-4 py-3 text-left text-sm'>
                        {formatDate(snapshot.created_at)}
                      </td>
                      <td
                        className='min-w-[200px] px-4 py-3 text-left text-sm'
                      ...
                      <td className='min-w-[80px] px-4 py-3 text-left text-sm'>
                        {snapshot.actor_name || 'System'}
                      </td>
                      <td
                        className='min-w-[250px] px-4 py-3 text-left text-sm'
                        title={snapshot.meta?.filename || '-'}>
                      ...
                      <td className='min-w-[80px] px-4 py-3 text-center'>
                        <button
                          ...
                          className='inline-flex items-center gap-1 rounded-md bg-red-50 px-3 py-1 text-xs whitespace-nowrap text-red-700 hover:bg-red-100'>
                          <FaUndo size={12} />
                          롤백
                        </button>
                      </td>
