...
const HRPage: React.FC = () => {
    ...
    useEffect(() => {
        if (employeesData?.data) {
            console.log("Received employees data:", employeesData.data);
            ...
            const mapped = employeesData.data.map((emp: any) => ({
                id: emp.id,
                name: emp.username,
                position: emp.role,
                department: '', // 백엔드에 없으므로 빈 값
                email: emp.email,
                phone: emp.contact,
                status: emp.status,
                hire_date: emp.date_joined,
                created_at: '', // 백엔드에 없으므로 빈 값
                updated_at: '', // 백엔드에 없으므로 빈 값
            }));
            setEmployees(mapped);
        }
    }, [employeesData]);
    ...
    const filteredEmployees = employees.filter((employee) => {
        ...
        const matchesPosition =
            positionFilter === "" || positionFilter === "전체"
                ? true
                : employee.position === positionFilter;

        // 상태 필터링
        const matchesStatus =
            statusFilter === "" ? true : employee.status === statusFilter;
    ...
    const EmployeeCard: React.FC<{ employee: Employee }> = ({ employee }) => {
        console.log("EmployeeCard", employee);
        ...
        const handleTerminateEmployee = async () => {
            if (
                window.confirm(`${employee.name} 직원을 퇴사 처리하시겠습니까?`)
            ) {
                try {
                    ...
                    <div className="ml-4 flex-1">
                        ...
                        <div className="mt-1">
                            <p className="text-sm text-gray-600 flex items-center">
                                <FiUser className="w-3 h-3 mr-2 text-gray-600" />{" "}
                                {employee.role}
                            </p>
                            <p className="text-sm text-gray-600 flex items-center">
                                <FiCalendar className="w-3 h-3 mr-2 text-gray-600" />{" "}
                                입사일: {employee.date_joined}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex items-start gap-4">
                    ...
                    <div className="min-w-32 flex flex-col gap-1">
                        ...
                        <SelectInput
                            ...
                            onChange={(value) =>
                                setPositionFilter(value === "전체" ? "" : value)
                            }
                        />
                    </div>
